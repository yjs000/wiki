...
[Codex App 소개](https://openai.com/index/introducing-the-codex-app/)는 앱을 여러 에이전트를 병렬로 관리하는 command center로 설명한다. 프로젝트별 thread, 내장 worktree, thread 안의 diff 검토, Skills, Automations, review queue가 함께 제공된다. [Codex App 문서](https://developers.openai.com/codex/app/)와 [CLI 문서](https://developers.openai.com/codex/cli/)를 보면 앱과 CLI는 같은 Codex 생태계를 공유하지만 사용자 감독과 상태 표현 방식이 다르다.
...
세 도구 모두 파일을 읽고 Agent를 실행하며 외부 시스템과 통합할 수 있다. 기능표만 보면 경계가 흐려진다. 다음 행동의 근거와 상태 단위를 보면 차이가 선명해진다.
...
| 정보 | 유일한 원본 | 이유 |
|---|---|---|
| 요구사항과 승인 | GitHub Issue | 실행 도구와 무관한 작업 계약 |
| 대화와 의사결정 | Hermes session + 승인 기록 | 사용자와의 의미 기반 상호작용 |
...
```json
{
  ...
  "approval": {
    ...
    "approved_by": "github:yjs000"
  }
}
```

Issue 생성은 idempotent해야 한다. Hermes가 응답을 못 받은 뒤 같은 승인 요청을 다시 처리해도 중복 Issue가 생기지 않아야 한다.
...
def issue_fingerprint(project_slug: str, goal: str, criteria: list[str]) -> str:
    canonical = "\n".join([
        ...
        goal.strip(),
        *sorted(item.strip() for item in criteria),
    ])
    return sha256(canonical.encode("utf-8")).hexdigest()
```

### 7.4 Orchestra Pipeline 예시

아래 YAML은 개념 예시다. 실제 필드명은 [Orchestra Pipeline 문서](https://docs.getorchestra.io/docs/core-concepts/pipelines)와 워크스페이스에서 내보낸 YAML을 기준으로 검증해야 한다.

```yaml
version: v1
name: Approved Issue Delivery
pipeline:
  delivery_group:
    name: delivery
    tasks:
      validate_contract:
        name: Validate issue contract
        integration: GITHUB
        integration_job: GET_ISSUE
        parameters:
      ...
      run_agent:
        ...
        parameters:
          ...
          prompt: ${{ outputs.validate_contract }}
        depends_on:
          - validate_contract

      verify_pr:
        name: Verify PR and required checks
        integration: GITHUB
        integration_job: GET_PULL_REQUEST
        parameters:
          repository: ${{ inputs.repository }}
          pull_request: ${{ outputs.run_agent.pull_request }}
        depends_on:
          - run_agent
...
실제 프로젝트에서는 액션 버전을 commit SHA로 고정하고, 최소 권한, 보호 브랜치, 필수 체크를 함께 설정해야 한다.
...
sequenceDiagram
    ...
    User->>Hermes: 작업 승인
    Hermes->>GitHub: 계약이 포함된 Issue 생성
    GitHub->>Orchestra: 승인 라벨 또는 webhook
    Orchestra->>Agent: Issue와 저장소 컨텍스트로 실행
...
```json
{
  ...
  "evidence": {
    "pull_request": "https://github.com/yjs000/law-rag/pull/123",
    "check_run": "https://github.com/yjs000/law-rag/actions/runs/456",
...
Codex App은 공식 문서에서 시스템 수준 sandbox와 권한 규칙을 설명한다. Orchestra Runtime 문서는 격리 실행, 자격 증명 주입, 감사 기록을 강조한다. Hermes는 여러 도구와 메시징 채널을 연결하므로 도구별 최소 권한과 사용자 승인 경계를 별도로 정의해야 한다.
...
1. 같은 작업을 Codex App, CLI, Hermes에서 실행하는 통제 실험
2. `law-rag` 현재 구조와 이 글의 Stage 모델 간 gap 분석
...
| 생명주기 | `status`, `version`, `created`, `updated`, `last_verified` | 초안·검토·완료와 정보 신선도 추적 |
| 책임 | `authors`, `reviewers`, `license` | 작성·검토·재사용 책임 명시 |
