---
title: "모델보다 하니스: Codex, Hermes, Orchestra로 살펴본 AI 작업 환경과 파이프라인"
description: "끊기는 에이전트 루프를 출발점으로 Codex, Hermes, Orchestra의 역할을 비교하며 AI 작업 환경과 파이프라인을 어떻게 바라볼지 탐구한다."
author: yjs000
published: 2026-07-20
updated: 2026-07-20
reading_time: 약 22분
tags: [harness-engineering, ai-agents]
---

# 모델보다 하니스: Codex, Hermes, Orchestra로 살펴본 AI 작업 환경과 파이프라인

| 작성자 | 게시·수정일 | 읽는 시간 | 태그 |
|---|---|---|---|
| yjs000 | 2026-07-20 | 약 22분 | Harness Engineering · AI Agents |
...
- [4. 세 도구의 역할](#4-세-도구의-역할)
- [5. 다음 행동을 누가 결정하는가](#5-공통점보다-중요한-차이-다음-행동을-누가-결정하는가)
...
선택은 사용 규모보다 **필요한 책임 범위**에 따라 달라진다.
...
- Codex에도 Skills와 Automations가 있다.
- Hermes에도 Cron, Delegation, Kanban이 있다.
- Orchestra Runtime에도 Git 접근과 Agent가 있다.

따라서 제품 이름보다 **어떤 시스템이 어떤 상태의 원본을 맡는가**를 먼저 정해야 한다.

### 질문의 시작

> Codex에서는 잘 이어지던 코딩이 왜 같은 OpenAI 계열 모델을 연결한 Hermes에서는 자주 끊길까?

- **프로젝트 경계:** Discord에서 여러 프로젝트를 운영하면 맥락이 섞였다.
- **다음 행동:** 하위 에이전트가 끝난 뒤 무엇을 이어서 할지 정해지지 않았다.
- **상태와 근거:** 진행 상황, 중간 결과, 실패 원인이 충분히 남지 않았다.

분석을 거치며 문제의 초점이 모델에서 하니스로 이동했다.

- **하니스:** 모델을 둘러싼 실행 환경 전체
- **컨텍스트:** 무엇을 읽고 기억하게 할지
- **도구:** 무엇을 어떤 형식과 권한으로 실행하게 할지
- **상태:** 진행 상황과 다음 행동을 어디에 저장할지
- **검증과 복구:** 완료를 무엇으로 증명하고 실패 후 어디서 재개할지

이 글은 공식 문서와 도구 비교, `law-rag`에서 Codex를 활용한 경험을 바탕으로 AI 작업 환경과 파이프라인을 이해해 가는 학습 기록이다.
...
실패는 하나의 원인만 갖지 않을 수 있다. 그래서 다음 정보를 나눠 기록한다.

- **주 원인:** `primary_failure_class`
- **기여 요인:** `contributing_factors[]`
- **관측 위치:** 어느 계층에서 발견했는가
- **책임과 복구:** 누가 고치며 재시도할 수 있는가

분류 기준은 발견 위치가 아니라 **무엇을 고쳐야 해결되는가**다. 테스트가 코드 결함을 정상적으로 발견했다면 `code`, 테스트가 빠졌거나 잘못된 성공을 냈다면 `verification`이다. 원인 사슬은 보존하되 재시도 책임은 한 계층에만 둔다.
...
[OpenAI의 Harness Engineering](https://openai.com/ko-KR/index/harness-engineering/) 사례가 보여 준 핵심은 모델보다 환경을 먼저 고쳐야 한다는 점이다.

- **사람의 역할:** 코드 작성에서 환경과 피드백 루프 설계로 이동
- **초기 병목:** Codex의 능력보다 부족한 도구, 추상화, 내부 구조
- **해결 방향:** "더 노력하라"는 지시 대신 읽고 실행할 수 있는 빌딩 블록 제공
...
[2026년 Codex App 소개](https://openai.com/index/introducing-the-codex-app/)는 앱을 여러 에이전트를 병렬로 관리하는 command center로 설명한다.

- **작업 분리:** 프로젝트별 thread와 내장 worktree
- **검토:** thread 안의 diff와 review queue
- **확장:** Skills와 Automations

[CLI 문서](https://developers.openai.com/codex/cli/)와 발표문을 함께 보면 App과 CLI는 Codex 생태계를 공유하지만 사용자 감독과 상태 표현 방식이 다르다. 제품 문서는 빠르게 이동하므로 App 기능 주장은 날짜가 명시된 발표문을 기준으로 삼았다.

같은 OpenAI 계열 모델을 선택해도 작업 환경까지 같아지지는 않는다.

- **Hermes:** 자체 시스템 프롬프트, 도구, 세션, 압축, gateway, delegation 정책으로 모델을 운전한다.
- **Codex App 기능:** 프로젝트, thread, worktree, review UI는 Hermes에 자동으로 따라오지 않는다.
- **Hermes 기능:** Discord, Telegram, 여러 provider, MCP, memory도 Codex App에 그대로 포함되지 않는다.
...
- 저장소 탐색, 파일 수정, 명령 실행, 결과 검증에 집중한다.
- App은 여러 thread와 worktree를 한 UI에서 감독한다.
- Skills는 문서·디자인·배포 작업으로 범위를 넓힌다.
- Automations는 반복 작업을 review queue로 보낸다.
...
[공식 문서](https://hermes-agent.nousresearch.com/docs/)와 [공식 저장소](https://github.com/NousResearch/hermes-agent)에서 확인되는 핵심 기능은 다음과 같다.

- **모델 연결:** provider 교체
- **실행 도구:** 파일, 터미널, 웹, Skills
- **지속 작업:** memory, delegation, cron
- **외부 연결:** MCP, Discord, Telegram

세부 동작은 [Tools Reference](https://hermes-agent.nousresearch.com/docs/reference/tools-reference), [Messaging](https://hermes-agent.nousresearch.com/docs/user-guide/messaging/), [Memory](https://hermes-agent.nousresearch.com/docs/user-guide/features/memory), [MCP](https://hermes-agent.nousresearch.com/docs/user-guide/features/mcp), [Delegation](https://hermes-agent.nousresearch.com/docs/user-guide/features/delegation)에서 확인할 수 있다.
...
[Orchestra Runtime Overview](https://docs.getorchestra.io/docs/ai-agents/overview)는 Runtime을 AI Agent를 build, run, monitor하는 control plane으로 설명한다.

- **실행 환경:** sandbox와 Git 접근
- **에이전트 기능:** Skills와 metadata
- **운영:** alerting, monitoring, 세션·turn 감사 기록
- **배치:** 직접 대화하거나 Pipeline task로 실행

[Agents 문서](https://docs.getorchestra.io/docs/ai-agents/agents)의 공식 예제는 실패한 파이프라인을 진단하고 수정 PR을 여는 흐름을 보여준다.

Orchestra의 뿌리는 데이터와 워크플로 오케스트레이션에 있다.

- **Pipeline:** YAML 또는 UI로 정의하는 DAG
- **Run:** 실행 상태와 결과
- **Task:** 단계와 의존성
- **Alerting:** 실패와 운영 알림

세부 개념은 [Pipelines](https://docs.getorchestra.io/docs/core-concepts/pipelines), [Pipeline Runs](https://docs.getorchestra.io/docs/core-concepts/pipelines/pipeline-runs), [Tasks](https://docs.getorchestra.io/docs/core-concepts/tasks), [Alerting](https://docs.getorchestra.io/docs/core-concepts/alerting) 문서에 나뉘어 있다.
...
**승인의 원본: append-only approval ledger**

- 인증된 actor와 승인 시각을 기록한다.
- contract hash, 프로젝트, 만료·철회 상태, correlation ID를 보관한다.
- Issue의 `approval_ref`는 원본이 아니라 ledger를 가리키는 projection이다.
- 승인 후 contract hash가 바뀌면 기존 승인을 무효화한다.

Issue 본문의 `approved_by` 문자열만으로는 승인 사실을 신뢰할 수 없다.

**비신뢰 입력: Issue, 댓글, 저장소 문서, 실패 로그**

- 시스템 지시와 분리한다.
- JSON Schema로 허용 필드, 길이, 형식을 검사한다.
- 고정된 Agent prompt의 data 영역에만 넣는다.
- 입력 텍스트가 권한이나 실행 정책을 바꾸지 못하게 한다.

**검증 명령: 자유 형식 shell 대신 신뢰된 profile**

- `verification_profile`은 버전 관리되는 검증 계약을 가리킨다.
- runner는 shell 문자열을 합성하지 않고 고정 executable과 argv를 실행한다.
- sandbox는 workspace 밖 파일, secret, 임의 egress 접근을 차단한다.
...
**중복 방지:** 같은 승인 요청을 다시 처리해도 Issue는 하나만 생성되어야 한다. fingerprint는 빠르게 중복 후보를 찾는 키일 뿐, 의미적으로 같은 작업을 완벽하게 판정하지는 못한다.
...
canonical contract의 규칙은 짧고 명확해야 한다.

- 배열 순서는 보존하고 객체 key만 정렬한다.
- schema version이 다르면 다른 계약으로 취급한다.
- float와 알 수 없는 필드를 거부한다.
- 문자열 정규화 정책을 고정한다.
- ledger에는 hash와 승인 당시의 canonical payload를 함께 보관한다.

해시 계산만으로 동시 요청을 막을 수는 없다.

- `(approval_id, contract_hash)` 또는 fingerprint에 unique constraint를 건다.
- Issue 생성 전에 원자적으로 reservation을 만든다.
- 생성 직후 프로세스가 종료돼도 다음 실행에서 GitHub와 reservation을 대조해 복구한다.
...
아래 블록은 단계와 소유권을 설명하는 **비실행 의사코드**다.

- Orchestra에 import할 수 있는 YAML이 아니다.
- 실제 job 이름과 task output은 workspace integration에 따라 달라진다.
- 구현할 때는 [Pipeline YAML Schema Reference](https://docs.getorchestra.io/docs/core-concepts/pipelines/schema), [Agents의 `RUN_AGENT` 예제](https://docs.getorchestra.io/docs/ai-agents/agents), workspace에서 export한 YAML을 사용한다. Orchestra validator를 통과한 artifact를 완료 증거로 남긴다.
...
CI 예시의 기본 원칙은 다음과 같다.

- action은 2026-07-20에 확인한 full commit SHA로 고정한다.
- checkout credential을 유지하지 않는다.
- SHA 갱신은 Dependabot 또는 승인된 절차로 처리한다.
- Fork PR에는 secret, OIDC, 쓰기 권한, 배포 credential을 주지 않는다.
- 보호 브랜치와 필수 check는 workflow 밖의 repository ruleset으로 강제한다.

신뢰하지 않는 코드를 실행한다면 네트워크와 cache 정책도 별도로 검토해야 한다.
...
공식 기능의 근거는 제품별로 구분한다.

- **Codex:** [Codex App 소개의 보안 절](https://openai.com/index/introducing-the-codex-app/)은 시스템 수준 sandbox와 작업 범위 밖 명령의 권한 요청을 설명한다.
- **Orchestra:** [Runtime Overview](https://docs.getorchestra.io/docs/ai-agents/overview)는 격리 실행 계정, 런타임 자격 증명 주입, session·run·turn 감사 기록을 설명한다.
- **Hermes:** [Tools Reference](https://hermes-agent.nousresearch.com/docs/reference/tools-reference)와 [Messaging](https://hermes-agent.nousresearch.com/docs/user-guide/messaging/)은 여러 도구와 채널의 연결을 설명한다. 최소 권한과 승인 경계는 운영자가 별도로 정해야 한다.
...
6. 도구의 기능과 문서는 빠르게 바뀐다. `updated` 날짜 이후에는 공식 문서를 다시 확인해야 한다.
