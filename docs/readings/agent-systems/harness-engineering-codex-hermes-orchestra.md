...
- **설계 해석:** Codex, Hermes, Symphony의 체감 차이는 모델 이름보다 context, tool, 권한, 상태와 검증 계약의 차이에서 생길 수 있다.
- **미검증 가설:** 같은 모델을 사용해도 프로젝트 경계와 검증 루프가 더 명확한 환경이 완료율을 높이고 사람의 개입을 줄일 것이다.

다음은 이 가설을 확인하기 위한 **설계 예시**이며 특정 제품의 공식 benchmark가 아니다.
...
학습 목표는 [학습계획](../../learning-plans/README.md)에 흡수해 기록한다.
